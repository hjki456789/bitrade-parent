...
public class RechargeGTDJob
{
    ...
    public void job() throws Exception {
        final Coin coin = this.coinService.findOne("GTD");
        if (coin == null || coin.getEnablePool() == 0) {
            return;
        }
        this.logger.info("---------------GTD\u5145\u503c\u4efb\u52a1\u5f00\u59cb---------------------");
        final ChainLastBlock blockInfo = this.chainLastBlockService.findByAssetsName("GTD");
        if (blockInfo == null) {
            return;
        }
        this.logger.info("-----------------------------------------GTD\u5f53\u524d\u7cfb\u7edf\u533a\u5757\u9ad8\u5ea6\uff1a" + blockInfo.getLastBlockNum());
        ...
        this.logger.info("-----------------------------------------GTD\u5f53\u524d\u5b89\u5168\u533a\u5757\u9ad8\u5ea6\uff1a" + lastblock);
        if (lastSafeBlock > startblock) {
            this.logger.info("-----------------------------------------GTD\u83b7\u53d6\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e-------------------------------");
            try {
                final List<Erc20Transcation> transcationList = this.erc20Component.getTranscationListByAddress(startblock, lastSafeBlock, "0x46f4129bfca15f8807c8ef067afc2377562e9074");
                this.logger.info("-----------------------------------------GTD\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e\u83b7\u53d6\u5b8c\u6210-------------------------------" + transcationList.size());
                ...
                this.logger.info("-----------------------------------------GTD\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e\u83b7\u53d6\u5931\u8d25-------------------------------" + e2);
                e2.printStackTrace();
            }
    ...
    public static void main(final String[] args) {
        final RechargeGTDJob job = new RechargeGTDJob();
