...
public class RechargeAMCJob
{
    ...
    public RechargeAMCJob() {
        this.serviceName = "bitrade-market";
        this.logger = LoggerFactory.getLogger((Class)FinanceConsumer.class);
    }
    
    @Scheduled(fixedRate = 40000L)
    public void job() throws Exception {
        final Coin coin = this.coinService.findOne("AMC");
        if (coin == null || coin.getEnablePool() == 0) {
            return;
        }
        this.logger.info("---------------AMC\u5145\u503c\u4efb\u52a1\u5f00\u59cb---------------------");
        final ChainLastBlock blockInfo = this.chainLastBlockService.findByAssetsName("AMC");
        if (blockInfo == null) {
            return;
        }
        this.logger.info("-----------------------------------------AMC\u5f53\u524d\u7cfb\u7edf\u533a\u5757\u9ad8\u5ea6\uff1a" + blockInfo.getLastBlockNum());
        ...
        this.logger.info("-----------------------------------------AMC\u5f53\u524d\u5b89\u5168\u533a\u5757\u9ad8\u5ea6\uff1a" + lastblock);
        if (lastSafeBlock > startblock) {
            this.logger.info("-----------------------------------------AMC\u83b7\u53d6\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e-------------------------------");
            try {
                final List<Erc20Transcation> transcationList = this.erc20Component.getTranscationListByAddress(startblock, lastSafeBlock, "0xe2ded2365bd426b2078bc947d70cc5c0141a4e6f");
                this.logger.info("-----------------------------------------AMC\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e\u83b7\u53d6\u5b8c\u6210-------------------------------" + transcationList.size());
    ...
    public static void main(final String[] args) {
        final RechargeAMCJob job = new RechargeAMCJob();
