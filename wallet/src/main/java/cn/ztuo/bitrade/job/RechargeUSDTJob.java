...
public class RechargeUSDTJob
{
    ...
    public void job() throws Exception {
        ...
        this.logger.info("---------------USDT\u5145\u503c\u4efb\u52a1\u5f00\u59cb---------------------");
        final ChainLastBlock blockInfo = this.chainLastBlockService.findByAssetsName("USDT");
        if (blockInfo == null) {
            return;
        }
        this.logger.info("-----------------------------------------\u5f53\u524d\u7cfb\u7edf\u533a\u5757\u9ad8\u5ea6\uff1a" + blockInfo.getLastBlockNum());
        ...
        this.logger.info("-----------------------------------------\u5f53\u524d\u5b89\u5168\u533a\u5757\u9ad8\u5ea6\uff1a" + lastblock);
        if (lastSafeBlock > startblock) {
            this.logger.info("-----------------------------------------\u83b7\u53d6\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e-------------------------------");
            try {
                final List<Erc20Transcation> transcationList = this.erc20Component.getTranscationListByAddress(startblock, lastSafeBlock, "0xdac17f958d2ee523a2206206994597c13d831ec7");
                this.logger.info("-----------------------------------------\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e\u83b7\u53d6\u5b8c\u6210-------------------------------" + transcationList.size());
                if (transcationList != null && transcationList.size() > 0) {
                    for (final Erc20Transcation result : transcationList) {
                        if (result.getTo() != null) {
                            ...
                            final BigDecimal amount = Convert.fromWei(new BigDecimal(Double.toString(Double.parseDouble(result.getValue()))), Convert.Unit.MWEI);
    ...
    public static void main(final String[] args) {
        final RechargeUSDTJob job = new RechargeUSDTJob();
