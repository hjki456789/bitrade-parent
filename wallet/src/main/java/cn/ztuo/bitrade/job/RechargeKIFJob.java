...
public class RechargeKIFJob
{
    ...
    public RechargeKIFJob() {
        this.serviceName = "bitrade-market";
        this.logger = LoggerFactory.getLogger((Class)FinanceConsumer.class);
    }
    
    @Scheduled(fixedRate = 65000L)
    public void job() throws Exception {
        final Coin coin = this.coinService.findOne("KIF");
        if (coin == null || coin.getEnablePool() == 0) {
            return;
        }
        this.logger.info("---------------KIF\u5145\u503c\u4efb\u52a1\u5f00\u59cb---------------------");
        final ChainLastBlock blockInfo = this.chainLastBlockService.findByAssetsName("KIF");
        if (blockInfo == null) {
            return;
        }
        this.logger.info("-----------------------------------------KIF\u5f53\u524d\u7cfb\u7edf\u533a\u5757\u9ad8\u5ea6\uff1a" + blockInfo.getLastBlockNum());
        ...
        this.logger.info("-----------------------------------------KIF\u5f53\u524d\u5b89\u5168\u533a\u5757\u9ad8\u5ea6\uff1a" + lastblock);
        if (lastSafeBlock > startblock) {
            this.logger.info("-----------------------------------------KIF\u83b7\u53d6\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e-------------------------------");
            try {
                final List<Erc20Transcation> transcationList = this.erc20Component.getTranscationListByAddress(startblock, lastSafeBlock, "0x4e880d553904fb3be6c3e767b67e23111a5abe28");
                this.logger.info("-----------------------------------------KIF\u533a\u5757\u533a\u5757\u4ea4\u6613\u6570\u636e\u83b7\u53d6\u5b8c\u6210-------------------------------" + transcationList.size());
                if (transcationList != null && transcationList.size() > 0) {
                    for (final Erc20Transcation result : transcationList) {
                        if (result.getTo() != null) {
                            ...
                            BigDecimal amount = Convert.fromWei(new BigDecimal(Double.toString(Double.parseDouble(result.getValue()))), Convert.Unit.KWEI);
                            amount = new BigDecimal(amount.doubleValue() * 10.0);
    ...
    public static void main(final String[] args) {
        final RechargeKIFJob job = new RechargeKIFJob();
