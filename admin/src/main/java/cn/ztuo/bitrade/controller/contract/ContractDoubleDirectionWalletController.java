...
@RestController
@RequestMapping({ "contract/double-direction-wallet" })
public class ContractDoubleDirectionWalletController extends BaseAdminController
{
    @Autowired
    ...
    @AccessLog(module = AdminModule.CONTRACT, operation = "分页查找双仓合约账户列表")
    ...
    @AccessLog(module = AdminModule.CONTRACT, operation = "双仓合约账户充币")
    public MessageResult changeBalance(@SessionAttribute("ADMIN_MEMBER") final Admin admin, @RequestParam("id") final Long id, @RequestParam("changeBalance") final BigDecimal changeBalance) {
        ...
        final ContractDoubleDirectionWallet contractDoubleDirectionWallet = this.contractDoubleDirectionWalletService.findOne(id);
        if (contractDoubleDirectionWallet == null) {
            return MessageResult.error("钱包信息不存在!");
        }
        if (contractDoubleDirectionWallet.getCoin() == null) {
            return MessageResult.error("币种信息不存在!");
        }
        final int result = this.contractDoubleDirectionWalletService.changeBalance(contractDoubleDirectionWallet, changeBalance, adminId);
        if (result > 0) {
    ...
    public MessageResult alterIsLock(@RequestParam("id") final Long id, @RequestParam("isLock") final Integer isLock) {
        ...
        final int result = this.contractDoubleDirectionWalletService.updateIsLock(id, isLock);
        if (result > 0) {
            return this.success(this.messageSource.getMessage("SUCCESS"));
        }
        return MessageResult.error("FAIL!");
